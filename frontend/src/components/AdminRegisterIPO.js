...
function AdminRegisterIPO() {
    ...
    const handleSubmit = async (e) => {
        ...
        for (const key in formData) {
            if (formData[key] !== null) { // Only append if not null
                formPayload.append(key, formData[key]);
            }
        }

        // Handle empty strings for nullable fields (dates, prices, pdfs) if needed
        ...
        console.log('Register IPO attempt:', formPayload);

        try {
            const response = await fetch('/api/ipos/', {
                method: 'POST',
                body: formPayload, // FormData handles Content-Type automatically
            });

            if (!response.ok) {
                const errorData = await response.json();
                // More detailed error handling for specific fields might be needed
                const errorMessage = errorData.detail ? errorData.detail : JSON.stringify(errorData);
                throw new Error(`Failed to register IPO: ${errorMessage}`);
            }
            ...
            // Clear file inputs manually as setting null to state doesn't clear them in HTML
            document.getElementById('logo').value = null;
            document.getElementById('rhp_pdf').value = null;
            document.getElementById('drhp_pdf').value = null;
